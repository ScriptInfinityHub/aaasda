...
local function CheckQuest()
    ...
    if GetLevel() <= 199 then
        info.monname = "Bandit"
        elseif GetLevel() > 200 and GetLevel() <=399 then
        info.monname = "Monkey"
...
function Tp(Pos)
    ...
    pcall(function ()
        ...
        if quest.monname == "" then return end
    for i, v in pairs(workspace.Enemies:GetDescendants()) do
    if v.Name == quest.monname then

        local innerModel = v:FindFirstChild(quest.monname)

        if innerModel then
            local hum = innerModel:FindFirstChild("Humanoid")
